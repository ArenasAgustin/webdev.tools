...
export function useJsonFormatter(): FormatterHook {
  ...
  const format = useCallback(async (input: string, options?: Partial<FormatConfig>) => {
    ...
    if (formatResult.ok) {
      ...
      if (options?.autoCopy) {
        navigator.clipboard.writeText(formatResult.value).catch(() => undefined);
      }
      return { ok: true };
    } else {
  ...
  const minify = useCallback(async (input: string, options?: Partial<MinifyConfig>) => {
    ...
    if (minifyResult.ok) {
      ...
      if (options?.autoCopy) {
        navigator.clipboard.writeText(minifyResult.value).catch(() => undefined);
      }
      return { ok: true };
    } else {
  ...
  const clean = useCallback(async (input: string, options?: Partial<CleanConfig>) => {
    ...
    if (cleanResult.ok) {
      if (!cleanResult.value.trim()) {
        ...
        if (options?.autoCopy) {
          navigator.clipboard.writeText(cleanResult.value).catch(() => undefined);
        }
        return { ok: true };
      }
    } else {
